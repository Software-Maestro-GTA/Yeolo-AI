...
- `planner.md`가 보완한 요구사항 및 설계 명세를 기반으로 API 엔드포인트 및 비즈니스 로직, LangChain 에이전트의 기능 동작을 검증할 테스트 코드를 우선 작성합니다.
...
- **적용할 Skill**: `pytest-mocking` (pytest, pytest-mock 등을 활용한 테스트 및 외부 API 모킹 가이드)
...
2.  **테스트 범위 및 환경 파악**:
    - `tests/` 디렉토리 하위에 해당 기능/API를 검증할 파이썬 테스트 파일(예: `tests/test_*.py`)을 작성합니다.
3.  **API 및 LLM Mocking (pytest-mocking 준수)**:
    - 기획 단계에 지정된 API 명세를 검토하여 외부 API(예: LLM, 구글 제미나이 API 등)의 목 데이터를 반환하도록 모킹(pytest-mock의 `mocker.patch` 또는 `unittest.mock.patch` 등)을 작성합니다. Happy Path와 에러 시나리오를 각각 검증하는 목 데이터를 매핑합니다.
...
5.  **테스트 실패 검증**:
    - `uv run pytest` 명령어를 실행하고, 작성한 테스트가 **정상적으로 실패(Red)**하는 것을 직접 눈으로 확인합니다.
6.  **현황판 업데이트 및 토큰 인계**:
