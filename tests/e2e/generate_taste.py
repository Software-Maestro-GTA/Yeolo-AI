...
# 공통 테스트 페이로드 정의 (행동 데이터 메타데이터)
def get_test_behavior_payload() -> BehaviorAnalysisRequest:
...
# 실행 예시: uv run pytest tests/e2e/generate_taste.py
...
async def test_generate_taste_e2e():
    ...
    request_payload = get_test_behavior_payload()
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert "travelPurpose" in profile
        assert "travelPaceDensity" in profile
...
async def run_local_test():
    print("==================================================")
    print("🤖 로컬 Gemini API 기반 취향 분석 연동 테스트 시작")
    ...
    request_data = get_test_behavior_payload()
    print("\n🚀 행동 기반 취향 분석 파이프라인 스트림 시작...\n")
    ...
    try:
        async for event in analyze_behavior_stream(request_data):
            ...
            print("-" * 50)
        print("\n✅ 취향 분석 테스트가 성공적으로 완료되었습니다!")
    except Exception as e:  # noqa: BLE001
        print(f"\n❌ 분석 도중 에러가 발생했습니다: {e!s}")
