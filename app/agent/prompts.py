...
COURSE_GENERATION_PROMPT = ChatPromptTemplate.from_messages([
    ("system", (
        ...
        "   - 장소명(placeName) 필드는 괄호나 영문 병용 표기(예: '영문명 (한글명)' 또는 '한글명 (영문명)')를 엄격히 금지하며, 오직 정갈한 한글 단독 명칭으로만 작성하십시오.\n"
        "   - 장소 영문명(placeEngName) 필드에는 한글을 일절 기입하지 말고, 반드시 해당 장소의 현지 공식 영문/로마자 명칭(예: 'Musée du Louvre', 'Le Comptoir du Relais', 'Gyeongbokgung Palace' 등)을 명확하게 작성하십시오.\n"
